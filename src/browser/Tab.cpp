...
Tab::Tab(std::shared_ptr<IRequest> http, int window_width, const FontMetrics& metrics)
    : http_(std::move(http)), window_width_(window_width), metrics_(metrics), url_("http://localhost/") {}
...
void Tab::load(const Url &url) {
  ...
  if (url_.href() == "about:welcome") {
    body = "<html><head><title>Welcome</title></head><body><div style=\"text-align:center; padding:50px;\"><h1>Welcome in CSurfer</h1><p>Your lightweight, SOLID-powered browser.</p><hr><p>Visit the project on GitHub:</p><a href=\"https://github.com/mohamedhany01/csurfer\">mohamedhany01/csurfer</a><br><br><p><i>Enjoy surfing the solid way!</i></p></div></body></html>";
  } else {
    // Fetch page body
  ...
  if (body.empty()) {
    ...
    body = "<html><body><h1>Error Loading Page</h1><p>We couldn't reach <b>" + url_.href() + "</b>. Please check the URL and try again.</p></body></html>";
  }
  ...
  if (!root_) {
    ...
    body = "<html><body><h1>Parsing Error</h1><p>The page at <b>" + url_.href() + "</b> could not be parsed.</p></body></html>";
  ...
  // Layout
  document_ = std::make_unique<DocumentLayout>(root_.get(), metrics_, window_width_);
  document_->layout();
...
void Tab::click(int x, int y) {
  ...
  for (auto it = all.rbegin(); it != all.rend(); ++it) {
    ...
    if (x >= obj->x && x < obj->x + obj->width &&
        total_y >= obj->y && total_y < obj->y + obj->height) {
  ...
  while (!queue.empty()) {
    ...
    if (node->type() == LexemeType::Element) {
        ...
        if (el->tag() == "title" && !el->children().empty()) {
            const auto* text_node = dynamic_cast<const Text*>(el->children().front().get());
            if (text_node) return text_node->text();
        }
