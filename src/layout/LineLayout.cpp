...
void LineLayout::layout() {
  ...
  for (const auto &child : children_) {
    ...
    // Here we approximate based on child heights since child layout measured it.
    // To match BlockLayout's 1.25 multiplier:
    
    // As a simplification for now, since TextLayout height is just the bounding box
    // from TTF_SizeUTF8 (which includes ascent and descent), we'll do simple stacking.
    max_ascent = std::max(max_ascent, child->height);
  }

  for (auto &child : children_) {
    // Top-align text within the line for now, to keep it simple without full TTF metrics
    child->y = y; 
  }
